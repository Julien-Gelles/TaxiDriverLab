...
class SARSAAgent:
    """
    On-policy tabular SARSA for the 2-passenger Taxi environment.
    The next action is sampled inside update() using the current policy,
    so no runner-level change is required.
    ...
    def update(self, state: int, action: int, reward: float, next_state: int, done: bool, **kwargs):
        ...
        # On-policy: sample next action using the current epsilon-greedy policy
        next_action = self.get_action(next_state, explore=self.epsilon > self.epsilon_min,
