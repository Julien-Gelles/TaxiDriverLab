...
def train_agent(
    ...
    """
    Train DQNAgent on the 2-passenger Taxi environment.
    State is the 18-dim float32 feature vector from env.reset()/step().
    ...
    for ep in range(num_episodes):
        ...
        state, _ = env.reset()   # 18-dim feature vector
        ...
        if verbose and (ep + 1) % 100 == 0:
            w = min(100, ep + 1)
            print(f"[DQN] Ep {ep+1}/{num_episodes} | "
