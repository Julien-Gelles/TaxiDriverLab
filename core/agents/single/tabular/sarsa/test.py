"""
Evaluation (testing) for SARSA agent on Taxi-v4 environment.

Runs episodes without exploration (greedy policy) and returns
evaluation metrics. Identical to Q-Learning testing since both
use the learned Q-table in the same way during evaluation.
"""
...
def test_agent(
    ...
    For SARSAAgent, epsilon is set to 0 during evaluation.

    Args:
        agent: SARSA agent to evaluate.
