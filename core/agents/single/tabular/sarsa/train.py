"""
Training loop for SARSA agent on Taxi-v4 environment.

Runs episodic training with the provided SARSA agent and returns metrics
(mean reward, mean steps) over the training run.

Key difference from Q-Learning: chooses next action before updating,
then uses that action in the update (on-policy learning).
"""
...
def train_agent(
    ...
    """
    Run the training loop for a fixed number of episodes using SARSA.
    ...
    SARSA (on-policy): chooses the next action before updating, then uses
    that action in the Q-value update. This learns the value of the policy
    being followed, not the optimal policy.

    Args:
        agent: SARSA agent to train.
    ...
    for episode in range(num_episodes):
        ...
        # SARSA-specific: choose action BEFORE the loop
        ...
        while True:
            ...
            # SARSA-specific: choose next action BEFORE update
            next_action = agent.choose_action(next_state, explore=True)

            # Update with 5 elements: s, a, r, s', a' (the action actually taken)
