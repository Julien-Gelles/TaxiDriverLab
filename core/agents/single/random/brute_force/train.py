"""
Boucle d'entraînement pour l'agent brute force (politique aléatoire).

Aucune mise à jour de paramètres : on exécute simplement des épisodes
...
def train_agent(
    ...
    """
    Exécute num_episodes épisodes avec l'agent aléatoire (aucun apprentissage).

    Chaque épisode se termine à la fin de l'épisode (terminated/truncated).
    Retourne les métriques pour comparaison avec le Q-Learning.
    ...
    Returns:
        (rewards, steps, epsilons, stopped_early). epsilons est une liste de 1.0 (compatibilité).
    """
    rewards: list[float] = []
    steps: list[int] = []
    epsilons: list[float] = []
