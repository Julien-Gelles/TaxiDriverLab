...
Taxi-v3) and the React dashboard (`app/`). It runs a simulation in a background
thread and streams the **decoded** environment state (option B) so the browser
...
- `GET  /layout` → static Taxi-v3 map (rows, cols, locations, walls)
...
`passenger`: `0–3` = waiting at location R/G/Y/B, `4` = inside the taxi.
`destination`: `0–3` = target location.
